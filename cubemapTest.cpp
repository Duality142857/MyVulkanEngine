...
struct UBO_scene
{
    ...
    alignas(16) MyGeo::Mat4f modelview_inv;
...
private:
    ...
    std::vector<MyUniformBuffer> uniformBuffers_skybox;

    UBO_scene ubo_scene{};
    UBO_shadow ubo_shadow{};
    UBO_skybox ubo_skybox{};
    ...
    BoxModel skybox{rhi,{500,500,500}};
    ...
    CubeMapTexture cubemapTexture{rhi};
...
public:
    CubemapTest(MyWindow* _mywindow, MyRenderer* _renderer):App{_mywindow,_renderer}
    {
        ...
        // texturePool.addTexture
        cubemapTexture.loadCubemap("../resources/cubemap/1.ktx");//vulkan space array yokohama_rgba
    ...
    {    
        ...
        //! skybox pass commands
        // {
        //     VkRenderPassBeginInfo skyboxRenderPassBI= renderpassBeginInfo(rhi->renderPass,rhi->framebuffers[imageIndex],rhi->swapChainExtent);
        //     std::array<VkClearValue, 2> clearValues{};
        //     clearValues[0].color={ { 0.025f, 0.025f, 0.025f, 1.0f } };
        //     clearValues[1].depthStencil={1.f,0};
        //     skyboxRenderPassBI.clearValueCount=2;
        //     skyboxRenderPassBI.pClearValues=clearValues.data();

        //     mycommandbuffer.beginRenderPass(&skyboxRenderPassBI);
        //     auto viewport=viewport_vk(rhi->swapChainExtent);
        //     auto rect=rect2D_vk(rhi->swapChainExtent);
        //     mycommandbuffer.setViewPort(&viewport);
        //     mycommandbuffer.setScissor(&rect);
        //     mycommandbuffer.bindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout_skybox,0,1,&descriptorSets_skybox[imageIndex]);
        //     mycommandbuffer.bindPipeline(VK_PIPELINE_BIND_POINT_GRAPHICS,pipeline_skybox.pipeline);

        //     skybox.bind(mycommandbuffer);
        //     skybox.draw(mycommandbuffer);
...
int selectedEntityId=-1;
    ...
    void prepareData() override
    {
        ...
        VkDeviceSize bufferSize_skybox=sizeof(UBO_skybox);
        ...
        for(int i=0;i!=rhi->images.size();++i) 
        {
            ...
            uniformBuffers_skybox.push_back({rhi,bufferSize_skybox});
    ...
    void updateData(uint32_t imageIndex) override 
    {
        ...
        MyGeo::Vec3f camV{0,0,0};
        if(mywindow->currentKeyAction==GLFW_PRESS)
        {
            switch (mywindow->currentKey)
            {
            case GLFW_KEY_W:
            {
                camV=cam.lookdirection;
                break;
            }
            case GLFW_KEY_S:
            {
            ...
            case GLFW_KEY_A:
            {
                camV=-cam.lookdirection.cross(cam.updirection);
                break;
            }
            case GLFW_KEY_D:
            {
        ...
        cam.position+=camV*t_delta*10;
        cam.lookat=cam.position+cam.lookdirection;
        ...
        ubo_scene.modelview_inv=(ubo_scene.view*ubo_scene.model).inverse();

        ubo_skybox.view=ubo_scene.view;
        ubo_skybox.proj=ubo_scene.proj;
        ubo_skybox.modelview_inv=ubo_scene.modelview_inv;
        ...
        uniformBuffers_skybox[imageIndex].updateData(&ubo_skybox,sizeof(ubo_skybox));
    ...
    {
        vulkanUtils.createPipelineLayout(descriptorSetLayout,&pipelineLayout_skybox);
        ...
        pipeline_skybox.createSkyBoxPipeline("../shaders/skybox.vert.spv","../shaders/skybox.frag.spv",pipelineLayout_skybox,1);
    ...
    void updateDescriptorSets(MyCommandBuffer& mycommmandbuffer, uint32_t imageIndex) override
    {
        ...
        VkDescriptorBufferInfo uniformBufferInfo_skybox=descriptors.bufferInfo(uniformBuffers_skybox[imageIndex].buffer, sizeof(UBO_skybox));
        ...
        VkDescriptorImageInfo cubemapImageInfo=cubemapTexture.descriptorImageInfo();
        
        std::array<VkWriteDescriptorSet,5> descriptorWrites_scene{};
        ...
        descriptorWrites_scene[4].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        descriptorWrites_scene[4].dstSet = descriptorSets_scene[setIndex];
        descriptorWrites_scene[4].dstBinding = 4;
        descriptorWrites_scene[4].dstArrayElement = 0;
        descriptorWrites_scene[4].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        descriptorWrites_scene[4].descriptorCount = 1;
        descriptorWrites_scene[4].pImageInfo = &cubemapImageInfo;
        ...
        //skybox
        std::array<VkWriteDescriptorSet,2> descriptorWrites_skybox{};
        descriptorWrites_skybox[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        descriptorWrites_skybox[0].dstSet = descriptorSets_skybox[setIndex];
        descriptorWrites_skybox[0].dstBinding = 0;
        descriptorWrites_skybox[0].dstArrayElement = 0;
        descriptorWrites_skybox[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
        descriptorWrites_skybox[0].descriptorCount = 1;
        descriptorWrites_skybox[0].pBufferInfo = &uniformBufferInfo_skybox;

        descriptorWrites_skybox[1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        descriptorWrites_skybox[1].dstSet = descriptorSets_skybox[setIndex];
        descriptorWrites_skybox[1].dstBinding = 4;
        descriptorWrites_skybox[1].dstArrayElement = 0;
        descriptorWrites_skybox[1].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        descriptorWrites_skybox[1].descriptorCount = 1;
        descriptorWrites_skybox[1].pImageInfo = &cubemapImageInfo;

        vkUpdateDescriptorSets(rhi->device,descriptorWrites_skybox.size(),descriptorWrites_skybox.data(),0,nullptr);
    
    }

    //create descriptor pool given poolsizes
    ...
    void createDescriptorSets() override
    {
        ...
        poolsizes.push_back(descriptors.poolSize(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,20));

        descriptors.createDescriptorPool(poolsizes,3000);
        

        //!create descriptorSetLayout
        // binding 0: ubo
        // binding 1: ordinary textures
        // binding 2: shadow texture
        // binding 3: ssbo
        // binding 4: cubemap texture
        ...
        layoutBindings.push_back(descriptors.setLayoutBinding(4,1,VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT));
        ...
        descriptors.allocateDescriptorSets(setLayouts,descriptorSets_skybox);
    ...
    void updateUi() override
    {
        ...
        ImGui::SliderFloat("mipLevel",&ubo_skybox.mipLevel,0.f,10.f);
...
int main()
{
    ...
    CubemapTest app(&mywindow,&renderer);
    app.run();
}
...
static Mat4f rotationMatrix4f(const Vec3f& axis, float angle)
{
    Vec3f nAxis=axis;
    nAxis.normalize();
...
//v0 v1 should be normalized before
static Mat3f rotationMatrix3f(const Vec3f& v0, const Vec3f& v1)
{
    Vec3f nAxis=v0.cross(v1).normalVec();
    float C=v0.dot(v1);
...
static Mat4f rotationMatrix4f(const Vec3f& v0, const Vec3f& v1)
{
    Vec3f nAxis=v0.cross(v1).normalVec();
    float C=v0.dot(v1);
    float S=std::sqrt(1-C*C);
